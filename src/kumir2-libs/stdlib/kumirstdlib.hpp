...
inline String &operator+(String &s, const /*utf8*/ char *cs)
{
	...
	String wcs = Coder::decode(UTF8, std::string(cs), encodingError);
	s.append(wcs);
	return s;
}
...
struct FileType {
	...
	static const char *_() { return "siibbl"; }
	...
	void setName(const String &name) { fullPath = name; }
	String getName() const { return fullPath; }

	void setMode(OpenMode m) { mode = int(m); }
	OpenMode getMode() const { return OpenMode(mode); }

	SpecialType getType() const { return SpecialType(type); }
	void setType(SpecialType t) { type = int(t); }
...
public:
	...
	static void init() { error.clear(); }
	static void finalize() {}
	static const String &getError() { return error; }
	...
	static std::string toUtf8(const String &s)
	{
		std::string result;
		EncodingError encodingError;
		result = Coder::encode(UTF8, s, encodingError);
		return result;
	}
	...
	static std::string toAscii(const String &s)
	{
		std::string result;
		EncodingError encodingError;
		result = Coder::encode(ASCII, s, encodingError);
		return result;
	}
	...
	static StringList splitString(const String &s, const Char separator, bool skipEmptyParts)
	{
		StringList result;
		size_t prev_index;
		prev_index = 0;
	...
	static void abort(const String &err)
	{
		error = err;
		if (AbortHandler) {
			AbortHandler();
...
public:
	...
	static bool readBool(InputStream &is)
	{
		...
		bool yes = (YES.count(word) > 0);
		bool no  = (NO .count(word) > 0);
