...
Document DocBookFactory::parseDocument(
	const QMap<ModelType, QString> roleValues,
	const QUrl &url,
	QString *error
) const {
...
void DocBookFactory::filterByRoles(
	const QMap<ModelType, QString> &roles,
	ModelPtr root
) const {
	if (!root) {
...
bool DocBookFactory::endElement(
	const QString &namespaceURI,
	const QString &localName,
	const QString &qName
) {
...
ModelPtr DocBookFactory::createListOfEntries(
	ModelPtr root,
	ModelType resType,
	ModelType findType
) {
...
QMap<QString, ModelPtr> &DocBookFactory::updateListOfKeywords(
	ModelPtr root, QMap<QString, ModelPtr> &result
) {
