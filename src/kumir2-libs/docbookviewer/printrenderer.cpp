...
qreal PrintRenderer::pointsToMillimeters(qreal points)
{
...
qreal PrintRenderer::textWidth(const QString &text, const QFont &font)
{
	...
	qreal fontPointSize = font.pointSizeF();
	qreal fontPixelSize = fm.height();
	qreal metricsDotsPerPoint = fontPixelSize / fontPointSize;
	qreal textPixelWidth = fm.width(text);
	qreal textPointWidth = textPixelWidth / metricsDotsPerPoint;
...
void PrintRenderer::renderTextFrameItem(
	QPainter &painter,
