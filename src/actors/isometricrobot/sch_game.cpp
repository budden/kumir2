...
QString normalizeString(const QString &t)
{
	return t.simplified();
}

bool parceJSON(const QScriptValue &value, Game &game)
{
	if (!value.isObject()) {
		return false;
	}
...
QString addTwoLeadingTabs(const QString &t)
{
	return "\t\t" + t;
}

QString addBoundingQuotes(const QString &t)
{
	return "\"" + t + "\"";
}

QString generateJSON(const Game &game)
{
...
bool isEqual(const Game &a, const Game &b)
{
