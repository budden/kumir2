...
void Vodoley::resizeEvent(QResizeEvent *event)
{
	...
	QRectF sizeRect = QRectF(
		scene->sceneRect().topLeft(),
		QSize(
			qMax(scene->sceneRect().width(), scene->sceneRect().height()) + 43,
			qMax(scene->sceneRect().width(), scene->sceneRect().height())
		)
	);
...
void Vodoley::FillC()
{
	fprintf(stderr, "Filling C from %d \n", (int) Curfill[2]);
	mutex.lock();
	Curfill[2] = Csize();
	mutex.unlock();
	redraw();
	fprintf(stderr, "Filled C to %d \n", (int) Curfill[2]);
}
