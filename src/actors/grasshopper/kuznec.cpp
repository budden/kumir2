...
{
	bool LeftButtonFlag = (mouseEvent->button() == 0x00000001);
...
//сдвиг окна вправо
void KumKuznec::MoveRight()
{
	WindowX0 -= -SizeX / WindowZoom / 4;
	WindowRedraw();
}

//сдвиг окна влево
void KumKuznec::MoveLeft()
{
	WindowX0 += SizeX / WindowZoom / 4;
	WindowRedraw();
}
...
void KumKuznec::ToWindow(void)
{
	...
	if ((MaxX - MinX < 0.00001) && (MaxY - MinY < 0.00001))
		return;
...
void KumKuznec::ToDubl()
{
	...
	for (int i = 0; i < lines.count(); i++) {
		QLineF TmpLine = lines[i]->line();
		QPen TmpPen = lines[i]->pen();
		QGraphicsLineItem *TmpItem = new QGraphicsLineItem(TmpLine);
...
void KumKuznec::FromDubl()
{
	...
	for (int i = 0; i < linesDubl.count(); i++) {
		QGraphicsLineItem *TmpItem = linesDubl[i];
		const QLineF &TmpLine = TmpItem->line();
		const QPen &TmpPen = TmpItem->pen();
		const QColor &TmpColor = TmpPen.color();
...
{
	NetShowFlag = BoxNet->isChecked();
	SqwareNetFlag = BoxSqware->isChecked();

	bool ok0 = false, ok1 = false;

	StepX = eStepX->displayText().toFloat(&ok0);
...
void KumKuznec::closeEvent(QCloseEvent *event)
{
	...
	if (Kpult->getLibMode() || autoClose) {
		qDebug() << "Lib mode: " << Kpult->getLibMode() << " autoClose: " << autoClose;
		close();
		event->accept();
		return;
	}
