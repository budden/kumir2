...
void EditLine::paint(
	...
	painter->drawText(
		4, FIELD_SIZE_SMALL + 4, FIELD_SIZE_SMALL * 3, FIELD_SIZE_SMALL * 1.5,
...
{
	istemp = false;
	iconPath = QUrl::fromLocalFile(resDir.absoluteFilePath("btn_radiation.png"));
...
{
	istemp = true;
	iconPath = QUrl::fromLocalFile(resDir.absoluteFilePath("temp.png"));
	rad = QImage(iconPath.toLocalFile());
}
...
{
	if (getItem(roboRow, roboCol)->leftWall) {
		return false;
	}
	roboCol--;
	return true;
...
bool ConsoleField::goUp()
{
	...
	if (getItem(roboRow, roboCol)->upWall) {
		return false;
	}
	roboRow--;
	return true;
...
{
	qDebug() << "ROW" << roboRow << " Col" << roboCol;
	if (getItem(roboRow, roboCol)->downWall) {
		return false;
	}
	roboRow++;
	return true;
...
{
	if (roboCol == rows.at(roboRow).count() - 1) {
		return true;
	}
