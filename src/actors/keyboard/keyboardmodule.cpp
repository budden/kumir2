...
/* public static */ QList<ExtensionSystem::CommandLineParameter>
	KeyboardModule::acceptableCommandLineParameters()
{
	// See "src/shared/extensionsystem/commandlineparameter.h" for constructor details
	return QList<ExtensionSystem::CommandLineParameter>();
}

/* public slot */ void KeyboardModule::changeGlobalState(
...
/* public slot */ void KeyboardModule::reloadSettings(
...
{
	/* алг цел код клавиши */
	return buffer_.dequeue().kumirCode;
}
...
/* public slot */ Keycode KeyboardModule::runOperatorINPUT(const QString &s, bool &ok)
{
	...
	QString normalized = s.trimmed().toUpper();
	Keycode result;
	result.code = 0;
	ok = false;
...
#define D(x, y) QPair<QString,int>(QString::fromUtf8(x), static_cast<int>(QString::fromLatin1(y).at(0).toLatin1()))
	static const QPair<QString, int> CODES[] = {
		...
		C("INSERT", 45),
		C("DELETE", 46),
		C("F1", 112), C("F2", 113), C("F3", 114), C("F4", 115),
		C("F5", 116), C("F6", 117), C("F7", 118), C("F8", 119),
		C("F9", 120), C("F10", 121), C("F11", 122), C("F12", 123),
		D("Й", "Q"), D("Ц", "W"), D("У", "E"), D("К", "R"),
		D("Е", "T"), D("Н", "Y"), D("Г", "U"), D("Ш", "I"),
		D("Щ", "O"), D("З", "P"), D("Ф", "A"), D("Ы", "S"),
		D("В", "D"), D("А", "F"), D("П", "G"), D("Р", "H"),
		D("О", "J"), D("Л", "K"), D("Д", "L"), D("Я", "Z"),
		D("Ч", "X"), D("С", "C"), D("М", "V"), D("И", "B"),
		D("Т", "N"), D("Ь", "M"), C("", 0)
	};
#undef C
#undef D
	QString keyName = normalized.mid(3);
	for (size_t index = 0; 0 != CODES[index].second; index++) {
		if (CODES[index].first == keyName) {
			ok = true;
	...
	if (!ok && keyName.length() == 1) {
		char aKey = keyName.at(0).toLatin1();
		if ('\0' != aKey) {
			ok = true;
			result.code = aKey;
		}
