	...
	if (!keepKursor) {
		for (int i = 0; i < count; i++) {
			...
			if (textPos > 0) {
				...
				cursor->setColumn(
					doc->indentAt(cursor->row()) * 2 +
					doc->data_[cursor->row()].text.length()
				);
			}
		}
	}

	if (cursorRow <= (int) doc->linesCount()) {
...
ToggleLineProtectedCommand::ToggleLineProtectedCommand(
	TextDocument *Doc, int lineNo
) {
	this->doc = Doc;
	this->lineNo = lineNo;
}
...
void ChangeHiddenLineDelimeterCommand::redo()
{
	...
	for (uint i = 0; i < doc->linesCount(); i++) {
		if ((int) i < firstHiddenLineNo || firstHiddenLineNo == -1) {
...
ToggleCommentCommand::ToggleCommentCommand(
...
QDataStream &operator << (
	QDataStream &stream,
	const InsertBlockCommand &command
) {
	stream << command.row;
...
QDataStream &operator << (
	QDataStream &stream,
	const RemoveBlockCommand &command
) {
	stream << command.cursorRow;
	stream << command.cursorCol;
	stream << command.block;
...
QDataStream &operator << (
	QDataStream &stream,
	const ToggleLineProtectedCommand &command
) {
...
QDataStream &operator >> (
	QDataStream &stream,
	ToggleLineProtectedCommand &command
) {
	stream >> command.lineNo;
	return stream;
}
...
QDataStream &operator << (
	QDataStream &stream,
	const ChangeHiddenLineDelimeterCommand &command
)
{
...
QDataStream &operator >> (
	QDataStream &stream,
	ChangeHiddenLineDelimeterCommand &command
) {
	stream >> command.firstHiddenLineNo;
	QString s;
	stream >> s;
...
QDataStream &operator << (
	QDataStream &stream,
	const ToggleCommentCommand &command
) {
