	...
	default: {
		if (editor_->analizerInstance_) {
			...
			static const QList<LexemType> AllowedLexemsForFreeCursor
				= QList<LexemType>() << LxTypeComment << LxTypeDoc;

			int indentColumns = 2 * editor_->document()->indentAt(row_);
			uint textPos = qMax(0, int(column_) - indentColumns);

			bool hasProp = lineProp.size() > 0;
			if (!hasProp) {
				return false;
			}
...
void TextCursor::evaluateCommand(const KeyCommand &command)
{
	...
	if ((uint) prevRow != row_ || (uint) prevLines != editor_->document()->linesCount()) {
		...
		if (editor_->analizerInstance_ && editor_->analizerInstance_->helper()
			...
			if (0 <= prevRow && (uint) prevRow < editor_->document()->linesCount()) {
		...
		if (op == QTextCursor::NextCell) {
			...
			if (m == MM_Move) {
				...
				if (column_ <= (uint) indent && row_ == 0) {}
					...
					&& column_ > (uint) (indent + editor_->document()->textAt(row_).length())) {
				...
				} else if (column_ <= (uint) indent && row_ > 0) {
...
bool TextCursor::modifiesProtectedLiines() const
{
	...
	uint linesCount = editor_->document()->linesCount();
	if (linesCount > 0u) {
