...
void EditorPlane::mousePressEvent(QMouseEvent *e)
{
	...
	int editableAreaLeftBorder = leftTextAreaPosition();
	...
	delimeterRuleMousePressedPoint_ = marginMousePressedPoint_ =
		textMousePressedPoint_ = QPoint(-1000, -1000);
...
//    // Paint a plain background
	...
	if (analizer_ && Shared::PythonIndents == analizer_->plugin()->indentsBehaviour()) {
...
void EditorPlane::paintLineNumbers(QPainter *p, const QRect &rect)
{
	...
	for (uint i = startLine; i <= endLine; i++) {
		...
		if (highlightedTextLineNumber_  + 1 != 0 && highlightedTextLineNumber_ + 1 == (int) realLineNumber) {
...
//                : QColor(Qt::lightGray);
		...
		if (editor_->plugin_->teacherMode_) {
			...
			if (realLineNumber == (uint) highlightedLockSymbolLineNumber_ &&
...
{
	//const char* ns = source.toUtf8().constData();
...
void EditorPlane::tryCorrectKeyboardLayoutForLastLexem()
{
	...
	if (! analizer_ || ! helper_) {
		return; // has no information how to do it
	}
	if (analizer_->plugin()->primaryAlphabetIsLatin()) {
