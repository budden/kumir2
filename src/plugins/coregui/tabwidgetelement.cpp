...
{
	if (editorInstance_) {
		Shared::Analizer::Data data = editorInstance_->documentContents();
		QUrl url = data.sourceUrl;
		if (isCourseManagerTab()) {
			return tr("%1 (Course)").arg(courseTitle_).trimmed();
		} else if (url.isValid()) {
			QString fullPath = editorInstance_->documentContents().sourceUrl.toLocalFile();
			QString shortPath = QFileInfo(fullPath).fileName();
			QString title;
