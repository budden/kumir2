...
void StatusBar::setEditorKeyboardLayout(
	QLocale::Language lang,
	bool capslock, bool shift, bool alt
) {
...
QSize StatusBar::keyboardLayoutItemSize() const
{
	...
	int width = 2 * (12 + 1) + maxTextWidth;
	int height = qMax(14, textHeight);
	return QSize(width + 2 * ItemPadding, height);
}

QSize StatusBar::cursorPositionItemSize() const
{
	QString text = tr("Row: ww, Col.: ww");
	int textW = statusBarFontMetrics().width(text);
	return QSize(textW + 2 * ItemPadding, qMax(14, fontHeight()));
}
...
QSize StatusBar::counterItemSize() const
{
	...
	int textWidth = state_ == GS_Unlocked ? maxTextWidthEdit : textWidthOther;
	int height = qMax(14, textHeight);
#ifdef Q_OS_MAC
...
{
	const int HideKeyboardStatusThereshold = 520;
	const int HideLineNumberThereshold = 460;
	const int HideModeThereshold = 200;
	...
	if (state_ == GS_Unlocked) {
		QSize remaining = cursorPositionItemSize() + keyboardLayoutItemSize();
		QSize message = messageItemSize();
...
void StatusBar::paintModeItem(QPainter &p, int x)
{
	...
	QRect textRect(QPoint(x + ItemPadding + xoffset, (height() - fontHeight()) / 2),
...
void StatusBar::paintKeyboardItem(QPainter &p, int x)
{
	...
	QColor activeColor = palette().brush(QPalette::Active, QPalette::Text).color();
	QColor disabledColor = palette().brush(QPalette::Disabled, QPalette::Text).color();
	...
	QRect textRect(QPoint(x + 25 + ItemPadding, (height() - fontHeight()) / 2),
