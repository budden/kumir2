...
void OneSession::relayout(uint realWidth, size_t fromLine, bool headerAndFooter)
{
	...
	// 1. Main text
	Q_ASSERT(lines_.size() == (int) props_.size());
...
) const
{
	...
	QBrush selectionBackroundBrush = parent_->palette().brush(
		parent_->hasFocus() ? QPalette::Active : QPalette::Inactive,
		QPalette::Highlight
	);
	QColor selectedTextColor = parent_->palette().brush(
		parent_->hasFocus() ? QPalette::Active : QPalette::Inactive,
		QPalette::HighlightedText
	).color();
	QFontMetrics fm(utilityFont());
	uint height = fm.height();
	uint xx = topLeft.x();
...
uint OneSession::drawMainText(QPainter &p, const QPoint &topLeft, const QRect &dirtyRect) const
{
	...
	QColor bgColor = parent_->palette().color(QPalette::Base);
...
void OneSession::triggerTextSelection(const QPoint &fromPos, const QPoint &toPos)
{
	...
	for (size_t l = 0; l < visibleLines_.size(); l++) {
		...
		QString thisLineText = line.text.mid(line.from, line.to - line.from);
		QRect thisLineRect = QRect(
