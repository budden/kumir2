...
int Plugin::checkTaskFromConsole(int taskID)
{
	KumZadanie task;
	QString curDir = ".";
	...
	if (
		resultStream.device() != 0 &&
		resultStream.status() == QTextStream::Ok
	) { //If we can - we writes marks to file
...
void Plugin::nextField()
{
	...
	prevFld->setEnabled(field_no > 0);
	nextFld->setEnabled(
		(field_no + 1) < cur_task->minFieldCount() &&
		cur_task->minFieldCount() > 0
	);
}
...
)
{
	...
	QString languageName = analizer->languageName().toLower();
