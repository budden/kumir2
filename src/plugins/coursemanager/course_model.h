...
public:
	...
	QStringList getIsps() const { return ispsNeeded; }

	QStringList getEnvFiles() const { return envFiles; }

	int Chapter() const { return Chapter_id; }
	...
	int next() const { return nextId; }

	void setNext(int id) { nextId = id; }

	void addSubTask(KumTask *task) { subTasks.append(task); }

	int subTasksCount() const { return subTasks.count(); }
...
public:
	...
	int taskMark(int id) const { return taskMark(nodeById(id, root)); }
